...
# If LINES is true: return tupla with all lines.
def runretp( cmd, LINES = False, SHELL = False, STDOUT = subprocess.PIPE ):
	...
	if LINES:
		...
		for item in ret.communicate():
			out += item if item else b''
		return out
...
# https://stackoverflow.com/questions/76410893/how-to-create-virtual-env-in-python-programatically
print( f'Creating the virtual environment. Name: "{ VENV_NAME }".' )
...
if not os.path.isfile( DEFAULT_FILE ):
	print( f'Creating { DEFAULT_FILE } file.' )
	file = open( DEFAULT_FILE, 'w' )
	# -*- coding: UTF-8 -*-
	file.close()


print( 'Improving the Activate.ps1 file.' )
try:
	file = open( f'{ VENV_NAME }/Scripts/Activate.ps1', 'w' )
	file.write( r'''<#
.Synopsis
...
if (-not $Env:VIRTUAL_ENV_DISABLE_PROMPT) {
	...
	function global:prompt {
		Write-Host -NoNewline -ForegroundColor DarkGray "$( Get-Date -Format HH:mm:ss.fff ) "
		Write-Host -NoNewline "$([char]27)[32m$([char]27)[3m$_PYTHON_VENV_PROMPT_PREFIX$([char]27)[0m "
		# _OLD_VIRTUAL_PROMPT
		Write-Host -NoNewline -foregroundcolor green "$( $PWD -replace "^(.*)\\", "~\" )"

		# if is git folder
		if ( $( test-path -path $pwd'\.git' )) {
			$status = $( runret "git status" )
			$branch = $( $status -match "^On branch (\w+)" )
			# $branch = $( $status -match "^On branch (\w+)" )
			write-host -nonewline " - $([char]27)[96m$([char]27)[3mgit$([char]27)[0m$([char]27)[96m:( $( $matches[1] ) )"
			# write-host $status # $matches
		}
...
subprocess.check_call( f'echo { VENV_NAME }/Scripts/Activate.ps1 | clip', shell = True )
print( 'Não é possível ser feita a inicialização da venv por aqui.\n  Basta apertar CTRL+V, em seguida ENTER.' )


# print( 'Inicido o scritp para ser feita atualização e instalação dos módulos' )
