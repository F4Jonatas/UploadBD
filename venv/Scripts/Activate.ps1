...
if (-not $Env:VIRTUAL_ENV_DISABLE_PROMPT) {
	...
	function global:prompt {
		...
		# if is git folder
		# https://www.reddit.com/r/commandline/comments/1j8fft/know_your_git_status_from_your_bash_prompt/
		if ( $( test-path -path $pwd'\.git' )) {
			...
			# $update = '🗘↕'
			# git fetch --all
			# $update = "$( git log HEAD..origin/$branch --oneline )"
			...
			# Modified files
			$modified = $( $changes -match "(^(\s*|\t*)M+)" )
			if ( $modified.length -gt 0 ) {
				$modified = " $([char]27)[2m~" + $modified.length + "$([char]27)[0m"
			}
			else {
				$modified = ""
			}

			write-host -nonewline " $([char]27)[96m$([char]27)[3mgit$([char]27)[0m$([char]27)[96m:( $branch$modified $([char]27)[96m)$([char]27)[0m"
		}
...
# https://learn.microsoft.com/en-us/windows/console/console-virtual-terminal-sequences#span-idtextformattingspanspan-idtextformattingspanspan-idtextformattingspantext-formatting
function prompt {
